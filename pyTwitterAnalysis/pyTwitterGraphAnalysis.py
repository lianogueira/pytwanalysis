...
class pyTwitterGraphAnalysis:   
    ...
    # function receives nodes and edges files and returns an nwtworkx 
    # Parameters: nodes_file = file path and file name for netwrok nodes
    #             edge_file = file path and file name for netwrok edges with weight
    def loadGraphFromFile(self, edge_file):
        G = nx.Graph()
        #nodes = nx.read_adjlist(nodes_file)
        G = nx.read_edgelist(edge_file, data=(('weight',float),))

        #G.add_edges_from(edges.edges())
        #G.add_nodes_from(edges.nodes())
    ...
    def contract_nodes_commty_per(self, G, perc, comm_att='community_louvain', enforce_ext_nodes_conn_fl ='N', commty_already_calculated='N'):
    ...
    #remove self loops nodes, isolate nodes and exclude smaller components
    ...
    # export nodes and edges of a graph into a file
    def export_nodes_edges_to_file(self, G, node_f_name, edge_f_name):
                        
        nx.write_edgelist(G,  edge_f_name)
        np.savetxt(node_f_name, np.array(sorted(G.degree(), key=lambda x: x[1], reverse=True)), fmt="%s")
        
        
    # creates a subgraph for one node. subgraph contains all nodes connected to that node and their edges to each other
    ...
    #return a array of the top degree nodes based on parameter passed by user    
    ...
    #calculate cluster labels for a graph
    ...
    #calculate cluster labels for a graph using community_louvain
    ...
    #Parameters: af_matrix = Affinity matrix
    ...
    #Returns: nb_clusters = the optimal number of clusters by eigengap heuristic
    ...
    #removes edges of nodes with less than the given degree. (both nodes in the edge must be less than the given degree for us to remove the edge)
    ...
    # Contract nodes degree 1 in groups of the given number
    ...
    #print Graph measures to the screen and to a file 
    def print_Measures(self, G, blnCalculateDimater=False, blnCalculateRadius = False, blnCalculateExtremaBounding=False, blnCalculateCenterNodes=False, fileName_to_print = None):       
